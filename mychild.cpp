...
void MyChild::newFile()
{
    ...
    setWindowTitle(curFile+"[*]"+(" - CrzWord"));
...
bool MyChild::maybeSave()
{
    ...
    ret = QMessageBox::warning(this,("CrzWord"),tr("Document'%1'has been changed,Save it?").arg(userFriendlyCurrentFile()),QMessageBox::Save | QMessageBox::Discard | QMessageBox::Cancel);
...
bool MyChild::saveAs()
{
    QString fileName = QFileDialog::getSaveFileName(this, tr("Save As"),curFile,tr("HTML Document (*.htm *.html);;All Files (*.*)"));
    if (fileName.isEmpty())
        return false;
